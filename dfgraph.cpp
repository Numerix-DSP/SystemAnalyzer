...
//	wxMessageBox (_T("Got here"), "GraphDisplay::GraphDisplay", wxOK | wxICON_EXCLAMATION, NULL);
	...
	sprintf (DisplayString, "%1.3lf Hz", SampleRate / 4.0);
	DataGraph->SetCenterLabel (DisplayString);
	sprintf (DisplayString, "%1.3lf Hz", SampleRate / 2.0);
...
//	wxMessageBox (_T("Got here"), "GraphDisplay::OnGraphTypeChanged", wxOK | wxICON_EXCLAMATION, NULL);
	...
	else												// Frequency domain graph
	{
		...
		else if (SampleRate >= 1e3)
		{
			sprintf (DisplayString, "0 Hz    ");
			DataGraph->SetLeftLabel (DisplayString);
			sprintf (DisplayString, "%1.3lf KHz", SampleRate / (4.0 * 1000.0));
			DataGraph->SetCenterLabel (DisplayString);
			sprintf (DisplayString, "%1.3lf KHz", SampleRate / (2.0 * 1000.0));
			DataGraph->SetRightLabel (DisplayString);
...
void GraphDisplay::SetDataArray (double * gain)
{
	pData = gain;
}

void GraphDisplay::SetScaling (double scaling)
{
	Scaling = scaling;
}

void GraphDisplay::SetGraphType (int graphType)
{
	GraphType = graphType;
}

void GraphDisplay::SetSampleRate (double sampleRate)
{
	SampleRate = sampleRate;
}


void GraphDisplay::SetDataSetSize (int size)
{
